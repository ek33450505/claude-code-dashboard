...
export default function AgentOfficeStrip({ liveAgentNames }: AgentOfficeStripProps) {
  ...
  return (
    ...
    >
      {/* Agent desk slots — full width, no queue offset */}
      <div
        style={{
          display: 'flex',
          flexDirection: 'row',
      ...
      {/* Queue indicator — top-right overlay badge, outside flex flow */}
      <div
        style={{
          position: 'absolute',
          top: 8,
      ...
      >
        <span style={{ ...PIXEL_FONT, fontSize: 5, color: 'rgba(0,255,194,0.5)' }}>QUEUE</span>
        <AnimatePresence>
          {liveAgentNames.length === 0 ? (
            <span style={{ ...PIXEL_FONT, fontSize: 5, color: 'rgba(90,108,138,0.4)' }}>—</span>
          ) : (
      ...
      {/* Info popover — portalled to document.body */}
      {createPortal(
        <AnimatePresence>
          {selected && popoverPos && (() => {
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                    <span style={{
                      ...PIXEL_FONT, fontSize: 5, padding: '2px 6px', borderRadius: 4,
