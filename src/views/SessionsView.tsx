...
function HookEventRow({ event }: { event: HookEvent }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-tertiary)] transition-colors text-xs">
      <span className="text-[var(--text-muted)] tabular-nums shrink-0 w-20">
        {time}
      </span>
      <span className="font-medium text-[var(--accent)] shrink-0 w-28 truncate">
        {event.hook_type}
      </span>
      ...
      <span
        className={`shrink-0 w-16 text-right font-medium ${resultColor(event.result)}`}
      >
        {event.result ?? "—"}
      </span>
      ...
      {preview.length === 0 ? (
        ...
        <div>
          {preview.map((w) => (
            <div
              key={w.id}
              className="flex items-center gap-3 px-4 py-2 border-b border-[var(--border)] last:border-0 text-xs hover:bg-[var(--bg-tertiary)] transition-colors"
            >
              <span className="text-[var(--text-muted)] tabular-nums shrink-0 w-20">
                {fmtTime(w.timestamp)}
              </span>
              <span className="text-rose-300 truncate flex-1 font-mono">
                {w.unstaged_files ?? "—"}
              </span>
    ...
    <div className="space-y-6">
      ...
      <div className="md:hidden space-y-3">
        {isLoading &&
          Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl p-4 animate-pulse space-y-2"
            >
          ...
          <div className="px-4 py-12 text-center text-(--text-muted)">
            <div className="px-4 py-12 text-center">
              {searchQuery || projectFilter ? (
                <div className="text-(--text-muted)">No matching sessions</div>
              ) : (
                <>
      ...
      <div className="hidden md:block bg-[var(--bg-secondary)] rounded-xl overflow-hidden border border-[var(--border)]">
        ...
        <div className="overflow-x-auto">
          ...
          <div
            ref={parentRef}
            className="h-[600px] overflow-auto min-w-[860px]"
          >
            {/* Loading skeleton */}
            {isLoading && (
